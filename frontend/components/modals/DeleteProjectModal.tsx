...
export default function DeleteProjectModal({ slug, name, show, setShow, onDelete }: { slug: string, name: string, show: boolean, setShow: (show: boolean) => void, onDelete: () => void }) {
    ...
    async function onConfirm() {
        ...
        const success = await deleteProject(slug, name);
        if (success) {
            onDelete();
        } else {
    ...
    return (
        <CenterModal show={show} title={`Remove ${name}`} content={warning} onConfirm={onConfirm} confirmDisabled={confirmDisabled} confirmText='Remove' onHide={() => setShow(false)} errorText={errorText} />
    );
}