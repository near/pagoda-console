...
In this tutorial, you learned about the basics of a marketplace contract and how it works. You went through the [lib.rs](#librs) file and learned about the [initialization function](#initialization-function) in addition to the [storage management](#storage-management-model) model. 

You then went through the [nft_callbacks](#nft_callbacksrs) file to understand how to [list NFTs](#listing-logic). In addition, you went through some important functions needed for after you've listed an NFT. This includes [removing sales](#removing-sales), [updating the price](#updating-price), and [purchasing NFTs](#purchasing-nfts).

Finally, you went through the enumeration methods found in the [`sale_view`](#sale_viewrs) file. These allow you to query for important information found on the marketplace contract. 
...
### Progressing to a full project (coming soon!)

We're working on the ability to eject tutorials into full-fledged projects. This adds a mainnet key and removes the tutorial, allowing you to take this dapp all the way to production more conveniently. Be on the lookout for updates in [our weekly Developer Console discussions on GitHub](https://github.com/near/community/discussions).

{/** TODO add a section to "eject" the current tutorial project
...
- **Remove the Tutorial from the menu bar.** If you need to reference this again, you can check it out in [the NEAR documentation](https://docs.near.org/docs/tutorials/contracts/nfts/introduction).
